...
1. 仅为“开始执行日”到本周周五生成计划，开始执行日前的日期不要输出任务日。
2. 周六（day_of_week=5）和周日（day_of_week=6）不要输出任务日；如果输出也必须 tasks 为空。
3. 输出的 day_of_week 采用周一=0、周日=6；date 使用 ISO 日期（YYYY-MM-DD）。
4. 每天 3-5 个核心任务。周一至周五以高强度推进为主。
5. priority 取值严格为 "high"、"medium"、"low" 之一。每天至少 1 个 high 优先级任务，high 任务安排在当天靠前位置。
6. order_index 从 0 开始，每天内连续递增，反映推荐执行顺序。high 优先级的任务 order_index 靠前。
7. buffer_percent 为 15-20 之间的整数（建议工作日 15）。
8. 每天总 estimated_hours（含 buffer）不超过 10 小时（工作日）。
9. 任务之间应有逻辑递进关系——前一天的产出是后一天的输入，体现一周内的推进节奏。
...
要求：
1. 本周自然周按周一到周日计算；仅输出开始执行日当天及其之后到周五的天数。
2. 周六、周日不要输出任务日；如果你输出了周末，tasks 必须为空。
3. day_of_week 必须和 date 的真实星期一致（周一=0 ... 周日=6）。
4. 任务描述用中文，写成"动词 + 具体交付物"的形式（如"完成XX模块的API接口开发，含单元测试"）。
5. 验收标准的 metric_name 和 target_value 用中文。
6. 一周的任务应有递进逻辑：前期打基础 → 中期核心推进 → 后期收尾验收。
7. 只返回符合 schema 的 JSON 对象，不要有其他任何内容。
""".strip()
